...
def load_wav(path):
  return librosa.core.load(path, sr=20000)[0]


def save_wav(wav, path):
  wav *= 32767 / max(0.01, np.max(np.abs(wav)))
  print("saving wav to: ", path)
  scipy.io.wavfile.write(path, 20000, wav.astype(np.int16))
...
def inv_spectrogram_tensorflow(spectrogram):
  ...
  '''
  S = _db_to_amp_tensorflow(_denormalize_tensorflow(spectrogram) + 20)
  return _griffin_lim_tensorflow(tf.pow(S, 1.5))
...
def find_endpoint(wav, threshold_db=-40, min_silence_sec=0.8):
  window_length = int(20000 * min_silence_sec)
...
def _stft_parameters():
  n_fft = (1025 - 1) * 2
  hop_length = int(12.5 / 1000 * 20000)
  win_length = int(50 / 1000 * 20000)
...
def _build_mel_basis():
  n_fft = (1025 - 1) * 2
  return librosa.filters.mel(sr=20000, n_fft=n_fft, n_mels=80)
