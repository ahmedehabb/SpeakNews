...
class Tacotron():
  ...
  def initialize(self, inputs, input_lengths, mel_targets=None, linear_targets=None):
    ...
    with tf.variable_scope('inference') as scope:
      ...
      print('Initialized Tacotron model. Dimensions: ')
      print('  embedding:               %d' % embedded_inputs.shape[-1])
      print('  prenet out:              %d' % prenet_outputs.shape[-1])
      print('  encoder out:             %d' % encoder_outputs.shape[-1])
      print('  attention out:           %d' % attention_cell.output_size)
      print('  concat attn & out:       %d' % concat_cell.output_size)
      print('  decoder cell out:        %d' % decoder_cell.output_size)
      print('  decoder out (%d frames):  %d' % (hp.outputs_per_step, decoder_outputs.shape[-1]))
      print('  decoder out (1 frame):   %d' % mel_outputs.shape[-1])
      print('  postnet out:             %d' % post_outputs.shape[-1])
      print('  linear out:              %d' % linear_outputs.shape[-1])
