...
def train_abs_single(args, device_id):
    ...
    # using the fine tuned bert from extractive model
    ...
    if (args.sep_optim):
        # separate optimizer, we made it true if we will use bert encoder
        # so we will do 2 optimizers, one for encoder and one for decoder since encoder is pretrained so can overfit and decoder is from scratch so can underfit
...
def build_abstractive(args):
    ...
    symbols = {'BOS': tokenizer.vocab['[unused0]'], 'EOS': tokenizer.vocab['[unused1]'],
               'PAD': tokenizer.vocab['[PAD]'], 'EOQ': tokenizer.vocab['[unused2]']}
    return build_predictor(args, tokenizer, symbols, model, logger)
