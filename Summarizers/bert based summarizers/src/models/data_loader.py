...
class DataIterator(object):
    ...
    def preprocess(self, ex, is_test):
        src = ex['src']
        # The last token of the target summary is replaced with a special end-of-sentence token (represented by 2).
        tgt = ex['tgt'][:self.args.max_tgt_len][:-1]+[2]
        src_sent_labels = ex['src_sent_labels']
        segs = ex['segs']
        if(not self.args.use_interval):
            # if use_interval is false, sets all elements in segs to 0. This effectively treats the entire document as a single segment.
            segs=[0]*len(segs)
        clss = ex['clss']
        src_txt = ex['src_txt']
        tgt_txt = ex['tgt_txt']
        ...
        # Finds the index of the first sentence label that exceeds the maximum positional limit
        max_sent_id = bisect.bisect_left(clss, self.args.max_pos)
        # Truncates src_sent_labels and clss based on max_sent_id. Only the elements up to max_sent_id are retained.
...
def load_one_text(args, paragraph, device):
    from others.tokenization import BertTokenizer
    tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', do_lower_case=True)
    sep_vid = tokenizer.vocab['[SEP]']
    cls_vid = tokenizer.vocab['[CLS]']
